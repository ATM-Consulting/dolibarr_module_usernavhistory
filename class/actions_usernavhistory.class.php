...
class ActionsUserNavHistory extends \userNavHistory\RetroCompatCommonHookActions
{
	...
	public function doActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (in_array('globalcard', $aContext) && !empty($object->element) && !empty($object->id)) {
			...
			if (method_exists($object, 'getElementType')) { // Check for native method introduced in v20
				$elementType = $object->getElementType();
			} else { // Fallback for older versions or objects not inheriting CommonObject
				$elementType = $unh->getObjectElementType($object);
			}
